...
def get_bel_model(
    ...
    **kwargs,
    ...
    power_spectrum_tt = kwargs["fsigma_8"]**2 * power_spectrum_linear / kwargs["sigma_8"]**2 * np.exp(
        -wavenumber * (a1 + a2 * wavenumber + a3 * wavenumber**2)
    )
    
    power_spectrum_mt = kwargs["fsigma_8"] / kwargs["sigma_8"] * power_spectrum_linear * np.exp(-invkdelta * wavenumber - b * wavenumber**6)
...
def get_nonlinearbel_model(
    ...
    **kwargs,
    ...
    power_spectrum_mt *=  np.sqrt(kwargs["power_spectrum_non_linear"] / power_spectrum_linear)
