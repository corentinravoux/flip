...
def M_gg_0_0_0(sig_g):
    def func(k):
        return (
            (1 / 2)
            * mpm.pi
            * mpm.erf((1 / 2) * mpm.sqrt(2) * k * sig_g) ** 2
            / (k**2 * sig_g**2)
        )
...
def M_gg_0_0_1(sig_g):
    def func(k):
        return (
            ...
            * mpm.sqrt(2)
            * mpm.sqrt(mpm.pi)
            * mpm.exp(-1 / 2 * k**2 * sig_g**2)
            * mpm.erf((1 / 2) * mpm.sqrt(2) * k * sig_g)
            / (k**3 * sig_g**3)
            - 3
            / 4
            * mpm.pi
            * mpm.erf((1 / 2) * mpm.sqrt(2) * k * sig_g) ** 2
            / (k**4 * sig_g**4)
            + (9 / 4) * mpm.exp(-(k**2) * sig_g**2) / (k**4 * sig_g**4)
            - 9
            / 4
            * mpm.sqrt(2)
            * mpm.sqrt(mpm.pi)
...
def N_gg_0_0_1(theta, phi):
    return (15 / 4) * mpm.cos(2 * theta) + 5 / 4
...
def N_gg_0_2_0(theta, phi):
    return (15 / 4) * mpm.cos(2 * phi + theta) + 5 / 4
...
def N_gg_0_2_2(theta, phi):
    return (
        (75 / 28) * mpm.cos(2 * theta)
        + (75 / 28) * mpm.cos(2 * phi - theta)
        + (75 / 28) * mpm.cos(2 * phi + theta)
        - 25 / 28
    )
...
def N_gg_0_4_0(theta, phi):
    return (
        (225 / 32) * mpm.cos(4 * phi)
        + (135 / 224) * mpm.cos(2 * theta)
        + (225 / 112) * mpm.cos(2 * phi - theta)
        + (225 / 112) * mpm.cos(2 * phi + theta)
        + 135 / 112
    )
...
def M_gg_1_0_0(sig_g):
    def func(k):
        return -mpm.sqrt(2) * mpm.sqrt(mpm.pi) * mpm.exp(
            -1 / 2 * k**2 * sig_g**2
        ) * mpm.erf((1 / 2) * mpm.sqrt(2) * k * sig_g) / (
            k**3 * sig_g**3
        ) + mpm.pi * mpm.erf(
            (1 / 2) * mpm.sqrt(2) * k * sig_g
        ) ** 2 / (
            k**4 * sig_g**4
        )
...
def M_gg_1_0_1(sig_g):
    def func(k):
        return (
            ...
            * mpm.erf((1 / 2) * mpm.sqrt(2) * k * sig_g) ** 2
            / (k**4 * sig_g**4)
            + 3 * mpm.exp(-(k**2) * sig_g**2) / (k**4 * sig_g**4)
            + (3 / 2)
            * mpm.sqrt(2)
            * mpm.sqrt(mpm.pi)
            * mpm.exp(-1 / 2 * k**2 * sig_g**2)
            ...
            * mpm.erf((1 / 2) * mpm.sqrt(2) * k * sig_g) ** 2
            / (k**6 * sig_g**6)
            + (27 / 2) * mpm.exp(-(k**2) * sig_g**2) / (k**6 * sig_g**6)
            - 27
            / 2
            * mpm.sqrt(2)
            * mpm.sqrt(mpm.pi)
...
def M_gg_1_2_0(sig_g):
    def func(k):
        return (
            ...
            * mpm.erf((1 / 2) * mpm.sqrt(2) * k * sig_g) ** 2
            / (k**4 * sig_g**4)
            + (3 / 2) * mpm.exp(-(k**2) * sig_g**2) / (k**4 * sig_g**4)
            - 15
            / 4
            * mpm.sqrt(2)
            * mpm.sqrt(mpm.pi)
...
def M_gg_2_0_0(sig_g):
    def func(k):
        return (
            mpm.exp(-(k**2) * sig_g**2) / (k**4 * sig_g**4)
            - mpm.sqrt(2)
            * mpm.sqrt(mpm.pi)
            * mpm.exp(-1 / 2 * k**2 * sig_g**2)
            * mpm.erf((1 / 2) * mpm.sqrt(2) * k * sig_g)
...
def M_gg_2_0_1(sig_g):
    def func(k):
        return (
            ...
            * mpm.erf((1 / 2) * mpm.sqrt(2) * k * sig_g) ** 2
            / (k**6 * sig_g**6)
            + 9 * mpm.exp(-(k**2) * sig_g**2) / (k**6 * sig_g**6)
            - 9
            / 4
            * mpm.sqrt(2)
            * mpm.sqrt(mpm.pi)
            ...
            * mpm.erf((1 / 2) * mpm.sqrt(2) * k * sig_g) ** 2
            / (k**8 * sig_g**8)
            + (81 / 4) * mpm.exp(-(k**2) * sig_g**2) / (k**8 * sig_g**8)
            - 81
            / 4
            * mpm.sqrt(2)
            * mpm.sqrt(mpm.pi)
...
def M_gg_2_2_0(sig_g):
    def func(k):
        return (
            ...
            * mpm.erf((1 / 2) * mpm.sqrt(2) * k * sig_g) ** 2
            / (k**6 * sig_g**6)
            + (9 / 2) * mpm.exp(-(k**2) * sig_g**2) / (k**6 * sig_g**6)
            - 9
            / 2
            * mpm.sqrt(2)
            * mpm.sqrt(mpm.pi)
...
def N_gv_0_1_0(theta, phi):
    return -3 * mpm.cos(phi + (1 / 2) * theta)
...
def M_gv_0_1_1(sig_g):
    def func(k):
        return (
            ...
            * mpm.erf((1 / 2) * mpm.sqrt(2) * k * sig_g)
            / (k**2 * sig_g)
            - 50 * mpm.exp(-1 / 2 * k**2 * sig_g**2) / (k**3 * sig_g**2)
            + 25
            * mpm.sqrt(2)
            * mpm.sqrt(mpm.pi)
            * mpm.erf((1 / 2) * mpm.sqrt(2) * k * sig_g)
...
def N_gv_0_1_1(theta, phi):
    return -9 / 2 * mpm.cos(phi - 3 / 2 * theta) - 3 / 2 * mpm.cos(
        phi + (1 / 2) * theta
    )
...
def N_gv_0_3_0(theta, phi):
    return (
        -9 / 8 * mpm.cos(phi - 3 / 2 * theta)
        - 9 / 4 * mpm.cos(phi + (1 / 2) * theta)
        - 45 / 8 * mpm.cos(3 * phi - 1 / 2 * theta)
    )


def M_gv_1_1_0(sig_g):
    def func(k):
        return -100 / 3 * mpm.exp(-1 / 2 * k**2 * sig_g**2) / (
            k**3 * sig_g**2
        ) + (50 / 3) * mpm.sqrt(2) * mpm.sqrt(mpm.pi) * mpm.erf(
            (1 / 2) * mpm.sqrt(2) * k * sig_g
        ) / (
            k**4 * sig_g**3
        )
...
def M_gv_1_1_1(sig_g):
    def func(k):
        return (
            -100 / 3 * mpm.exp(-1 / 2 * k**2 * sig_g**2) / (k**3 * sig_g**2)
            - 25
            / 3
            * mpm.sqrt(2)
            * mpm.sqrt(mpm.pi)
            * mpm.erf((1 / 2) * mpm.sqrt(2) * k * sig_g)
            / (k**4 * sig_g**3)
            - 150 * mpm.exp(-1 / 2 * k**2 * sig_g**2) / (k**5 * sig_g**4)
            + 75
            * mpm.sqrt(2)
            * mpm.sqrt(mpm.pi)
            * mpm.erf((1 / 2) * mpm.sqrt(2) * k * sig_g)
...
def N_vv_0_2_0(theta, phi):
    return (9 / 2) * mpm.cos(2 * phi) + (3 / 2) * mpm.cos(theta)
